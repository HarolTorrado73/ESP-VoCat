# ESP-ESP_VoCat Rotating Base

[Español](README_ES.md) | **English**

## Project Introduction

This project is the firmware for the rotating base of Espressif's ESP_VoCat intelligent interactive device, developed based on the ESP32-C61 chip. The project implements precise stepper motor control, magnetic slide switch detection, UART communication, and other features, providing vivid interactive actions and sensitive input detection for smart devices.
...
- **Rich Event Detection**:
  ...
  - Pairing mode (PAIRING): Two ESP_VoCat devices face-to-face for pairing and networking
...
### UART Communication
- **UART Protocol Communication**: Data communication with ESP_VoCat
- **Command Types**:
...
- Pairing mode detection (two ESP_VoCat face-to-face for pairing)
...
2. **Magnetic Switch Automatic Calibration** (if first use):
   ...
   - After the system records the second position, send `0x12` to ESP_VoCat to notify second position calibration complete
   ...
   - After calibration is complete, send `0x13` to ESP_VoCat to notify calibration complete
...
The base periodically sends this command. Once ESP_VoCat is magnetically attached and successfully receives the command, it can confirm that the connection to the base has been established and then start playing the connection animation.
...
- `0x09`: Pairing mode (PAIRING) - Two ESP_VoCat face-to-face for pairing
- `0x10`: Start calibration program
- `0x11`: Calibration start notification
- `0x12`: Second position calibration complete (ESP_VoCat gives voice prompt for third step after receiving)
...
- ESP_VoCat can synchronize status according to this command
...
**Note**: This project is supporting firmware for Espressif's ESP_VoCat smart device, for learning and reference only.
